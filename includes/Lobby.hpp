...
public:
    ...
    std::unordered_map<int, int> playerColors; // map connection -> colorId

    Lobby(int id);
    ~Lobby();

    bool addConnection(int connection, const std::string& playerName, int colorId); // ajoute une connexion au lobby avec le nom du joueur et la couleur
    ...
    bool hasColor(int colorId) const; // vérifie si une couleur est déjà prise dans le lobby
    std::vector<std::pair<std::string, int>> getPlayerColors() const; // retourne la liste des joueurs avec leurs couleurs
};
